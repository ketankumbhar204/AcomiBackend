...
public interface MenuPlanningHistoryRepository extends JpaRepository<MenuPlanningHistoryEntity, UUID> {
    ...
    @Query(
            ...
            WHERE h.space.id = :spaceId
              ...
              AND h.deleted = false
              AND (
                    h.entryType = com.countin.countin_backend.meal.domain.model.MenuHistoryEntryType.COMBO
                    OR h.item IS NULL
                    OR NOT EXISTS (
                        SELECT 1 FROM com.countin.countin_backend.meal.infrastructure.persistence.entity.SpaceFoodItemSettingsEntity s
