...
public interface MealParticipationRepository extends JpaRepository<MealParticipationEntity, UUID> {
    ...
    @Query("""
            ...
            JOIN FETCH p.mealPlan
            WHERE p.space.id = :spaceId
              AND p.status = com.acomi.acomi_backend.meal.domain.model.MealParticipationStatus.ACTIVE
            """)
    ...
    @Query("""
            ...
            JOIN FETCH p.mealPlan
            WHERE p.space.id = :spaceId
              AND p.status <> com.acomi.acomi_backend.meal.domain.model.MealParticipationStatus.STOPPED
            """)
