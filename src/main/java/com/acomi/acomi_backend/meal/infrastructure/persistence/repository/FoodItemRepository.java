...
public interface FoodItemRepository extends JpaRepository<FoodItemEntity, UUID> {
    ...
    @Query("""
            ...
            WHERE i.isActive = true
              AND (
                  (i.scope = com.acomi.acomi_backend.meal.domain.model.FoodScope.GLOBAL
                   AND i.space IS NULL
                   AND NOT EXISTS (
                       SELECT 1 FROM SpaceFoodItemSettingsEntity s
                       WHERE s.spaceId = :spaceId AND s.itemId = i.id AND s.isEnabled = false))
                  OR (i.scope = com.acomi.acomi_backend.meal.domain.model.FoodScope.SPACE
    ...
    @Query("""
            ...
            JOIN FETCH i.category
            WHERE i.id = :id
              AND i.scope = com.acomi.acomi_backend.meal.domain.model.FoodScope.SPACE
              AND i.space.id = :spaceId
            """)
    ...
    @Query("""
            SELECT i FROM FoodItemEntity i
            WHERE i.scope = com.acomi.acomi_backend.meal.domain.model.FoodScope.GLOBAL
              AND i.name = :name
              AND i.isActive = true
            """)
    ...
    @Query("""
            SELECT COUNT(i) FROM FoodItemEntity i
            WHERE i.isActive = true
              AND i.scope = com.acomi.acomi_backend.meal.domain.model.FoodScope.GLOBAL
              AND i.space IS NULL
            """)
