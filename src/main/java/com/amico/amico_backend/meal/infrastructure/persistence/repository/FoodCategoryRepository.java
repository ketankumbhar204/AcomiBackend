...
public interface FoodCategoryRepository extends JpaRepository<FoodCategoryEntity, UUID> {
    ...
    @Query("""
            ...
            WHERE c.isActive = true
              AND (
                  (c.scope = com.amico.amico_backend.meal.domain.model.FoodScope.GLOBAL
                   AND c.space IS NULL
                   AND NOT EXISTS (
                       SELECT 1 FROM SpaceFoodCategorySettingsEntity s
                       WHERE s.spaceId = :spaceId AND s.categoryId = c.id AND s.isEnabled = false))
                  OR (c.scope = com.amico.amico_backend.meal.domain.model.FoodScope.SPACE
    ...
    @Query("""
            ...
            WHERE c.isActive = true
              AND c.scope = com.amico.amico_backend.meal.domain.model.FoodScope.GLOBAL
              AND c.space IS NULL
            """)
    ...
    @Query("""
            SELECT c FROM FoodCategoryEntity c
            WHERE c.id = :id
              AND c.scope = com.amico.amico_backend.meal.domain.model.FoodScope.SPACE
              AND c.space.id = :spaceId
            """)
