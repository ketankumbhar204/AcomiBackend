...
public interface SpaceMembershipRepository extends JpaRepository<SpaceMembershipEntity, UUID> {
    ...
    @Query("""
            ...
            WHERE sm.user.id = :userId
              AND sm.status = com.amico.amico_backend.member.domain.model.MembershipStatus.ACTIVE
              AND s.isActive = true
    ...
    @Query("""
            ...
            WHERE sm.space.id = :spaceId
              AND sm.status = com.amico.amico_backend.member.domain.model.MembershipStatus.ACTIVE
            ORDER BY CASE WHEN sm.role = com.amico.amico_backend.member.domain.model.MembershipRole.OWNER
    ...
    @Query("""
            UPDATE SpaceMembershipEntity sm
            SET sm.status = com.amico.amico_backend.member.domain.model.MembershipStatus.REMOVED,
    ...
    List<UUID> findManagedSpaceIdsByTypesAndRoles(
            ...
            @Param("spaceTypes") List<com.amico.amico_backend.space.domain.model.SpaceType> spaceTypes);
}
